...
{
    // distance-weighted linear: yakın komşu daha çok etkiler
...
double compute_sdot(double h_eff, double m_dot_g, double k_surf,
                    ...
                    double T_recovery)
{
    sd_Tw4 = T_ablation*T_ablation*T_ablation*T_ablation;
    sd_num = h_eff*(T_recovery - T_ablation)
           + m_dot_g*cp_g*(T1 - T_ablation)
           + emissivity*sigma_SB*(T_surr*T_surr*T_surr*T_surr - sd_Tw4)
           - k_surf*(T_ablation - T1)/dx_surf;
    sd_den = rho_char * Delta_H_melt;
    double sdot_raw = sd_num / sd_den;
    const double sdot_min = 1e-7;  // 0.1 um/s - bang-bang onleme
...
{
    cout << string(80, '=') << "\n";
    cout << "ABLASYON ISI TRANSFERI v12 (C++) - ITERATIF YUZEY\n";
    cout << string(80, '=') << "\n";
    ...
    for (int n = 1; n <= nstep; n++)
    {
        ...
        // =====================================================================
        // 1. PIROLIZ  (her adimda bir kez — T_old bazli)
        // =====================================================================
        for (int c = 0; c < N_comp; c++)
        {
        ...
        // =====================================================================
        // 3. BASINC  (her adimda bir kez — piroliz gibi sabit)
        // =====================================================================
        for (int i = 0; i < N_nodes; i++)
        {
        ...
        // =====================================================================
        // 4. BLOWING  (basinc cozumunden sonra, iterasyon disinda)
        // =====================================================================
        ...
        //
        // Her iterasyonda:
        //   a) STATE MACHINE → T_wall, s_dot   (T1 kullanarak)
        //   b) TDMA          → T_new            (T_wall kullanarak)
        //   c) T1 = T_new[1] guncelle
        //   d) Convergence: |dT1| < eps_T  AND  |dsdot| < eps_sdot
        // =====================================================================
        k_surf = 2.0*k_node[0]*k_node[1] / (k_node[0]+k_node[1]);

        double T1      = T_old[1];   // iterasyon baslangici: lagged deger
        double sdot_iter = s_dot;    // onceki adimin sdot'u ile basla
        int    iter_count = 0;
        ...
        for (int it = 0; it < max_iter; it++)
        {
            ...
            if (!ablation_active)
            {
                ...
                if (Tw_stat >= T_ablation)
                {
                    ...
                    if (sdot_new > 0.0) ablation_active = true;
                    else                sdot_new = 0.0;
                }
                else
                {
            ...
            else
            {
                ...
                else
                {
                    ...
                    T_wall_new = solve_Twall_NR(
                        h_eff, m_dot_g, k_surf, T1, T1,
                        T_recovery, emissivity, sigma_SB, T_surr, dx_surf, cp_g);
                    sdot_new = 0.0;
                }
            }
            ...
            double dsdot = fabs(sdot_new - sdot_iter);
        ...
        // 7. UPDATE
        // =====================================================================
        if (recession_step == 0.0) T_prev = T_old;
