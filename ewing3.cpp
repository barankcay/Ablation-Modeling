...
double interp1_linear(const vector<double>& x, const vector<double>& y, double xq)
{
    ...
    return lerp(y[i], y[i + 1], (xq - x[i]) / (x[i + 1] - x[i]));
}
...
// =============================================================================
struct ThermalTable { vector<double> T, cp, k, eps; };
ThermalTable tbl_v, tbl_c;

static void push_row_thermal(ThermalTable& tbl,
                              double T, double cp, double k, double eps)
{
    tbl.T.push_back(T); tbl.cp.push_back(cp);
    tbl.k.push_back(k); tbl.eps.push_back(eps);
}
...
{
    double eps_v = interp1_linear(tbl_v.T, tbl_v.eps, T);
    double eps_c = interp1_linear(tbl_c.T, tbl_c.eps, T);
...
// =============================================================================
struct PyroGasTable { vector<double> T, cp, mu; };
PyroGasTable tbl_pg;

static void push_row_pyro_gas(PyroGasTable& tbl,
                               double T, double cp, double mu)
{
    tbl.T.push_back(T); tbl.cp.push_back(cp); tbl.mu.push_back(mu);
}
...
// Piroliz isi: Q_p(T)  [J/kg]  — thermalProperties.csv'den
...
{
    ifstream fin(fname);
    if (!fin.is_open()) { printf("ERROR: '%s' acilamiyor!\n", fname.c_str()); exit(1); }
    ...
    printf("[TABLE] %s: %d Bg grubu, %d satir/grup\n",
           fname.c_str(), (int)bpt.bg.size(),
           bpt.bg.empty() ? 0 : (int)bpt.Tw[0].size());
}
...
static double row_interp(const vector<double>& row, double L)
{
    ...
    return lerp(row[j0], row[j1], idx - j0);
}
...
// f(L) = k/dx*(Tw(L)-T1) - h_eff*(T_rec-Tw)
//       - eps*sigma*(Tsurr^4 - Tw^4) - (h_eff/cp_g)*Tchem(L) = 0
// =============================================================================
                  ...
                  double dx_surf, double cp_g, double L_guess)
{
    ...
    for (int iter = 0; iter < 100; iter++)
    {
        ...
        L += step;
        L = max(1.0, min((double)bpt.Tw[0].size(), L));
...
// =============================================================================
// YARDIMCI: phi-agirlikli bulk yogunluk
// =============================================================================
static double calc_rho_bulk(double a0, double a1, double a2,
                            double Gamma,
...
{
    double rho_intr =
        Gamma * ( (rho_v_comp[0]-(rho_v_comp[0]-rho_c_comp[0])*a0)
                 +(rho_v_comp[1]-(rho_v_comp[1]-rho_c_comp[1])*a1) )
      + (1.0-Gamma)*( rho_v_comp[2]-(rho_v_comp[2]-rho_c_comp[2])*a2 );

    double alpha_intr = (rho_intr_v - rho_intr) / (rho_intr_v - rho_intr_c);
    alpha_intr = max(0.0, min(1.0, alpha_intr));
...
{
    cout << string(80, '=') << "\n";
    cout << "ABLASYON ISI TRANSFERI — Ewing NR-L + phi-agirlikli bulk yogunluk\n";
    cout << string(80, '=') << "\n";
    ...
    // Intrinsic (phi'siz) referans yogunluklar
    ...
    // Bulk (phi dahil) referans yogunluklar
    ...
    for (int n = 1; n <= nstep; n++)
    {
        ...
        //    phi-agirlikli: (1-phi(alpha)) * rho_intrinsic
        // =====================================================================
        for (int i = 0; i < N_nodes; i++)
        {
            ...
            // drho_dt: phi ile olceklenmus bulk kaynak terimi
        ...
        for (int i = 1; i < N_nodes-1; i++)
        {
            ...
            // Node-ozel harmonik ortalama permeability
        ...
        // =====================================================================
        // 6. ITERATIF YUZEY + SICAKLIK COZUMU
        // =====================================================================
        ...
        for (int it = 0; it < max_iter; it++)
        {
            ...
            // (i) mdot / h_eff: mevcut T_wall ile guncelle
            {
                double K_0    = K_v*(1.0-alpha_eff[0]) + K_c*alpha_eff[0];
                double K_1    = K_v*(1.0-alpha_eff[1]) + K_c*alpha_eff[1];
            ...
            for (int i = 1; i < N_nodes-1; i++)
            {
                ...
                // Piroliz kaynak terimi — Ewing formülasyonu (DUZELTILMIS)
                // S_pyro = -(Q_p(T) - h_pyro + cp_g*T) * (rho_v - rho_c) * d(alpha_eff)/dt
                ...
                // t_Spyro  = -(Q_p_T(T_old[i]) - t_hpyro + cp_g_T(T_old[i])*T_old[i])
        ...
        if (recession_step > 0.0)
        {
            ...
            for (int j = 0; j < N_nodes; j++)
            {
                ...
                k_node[j] = k_mix(T_new[j], alpha_eff[j]);
            }
            k_surf = 2.0*k_node[0]*k_node[1] / (k_node[0]+k_node[1]);
        }
