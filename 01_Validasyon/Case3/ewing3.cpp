...
{
    tbl_v = ThermalTable{};
    push_row_thermal(tbl_v, 256,  1000, 60, 0.8);
    push_row_thermal(tbl_v, 298,  1000, 60, 0.8);
    push_row_thermal(tbl_v, 444,  1000, 60, 0.8);
    push_row_thermal(tbl_v, 556, 1000, 60, 0.8);
    push_row_thermal(tbl_v, 644, 1000, 60, 0.8);
    push_row_thermal(tbl_v, 833, 1000, 60, 0.8);
    push_row_thermal(tbl_v, 1111,1000, 60, 0.8);
    push_row_thermal(tbl_v, 1389,1000, 60, 0.8);
    push_row_thermal(tbl_v, 1667,1000, 60, 0.8);
    push_row_thermal(tbl_v, 1944,1000, 60, 0.8);
    push_row_thermal(tbl_v, 2222,1000, 60, 0.8);
    push_row_thermal(tbl_v, 2778,1000, 60, 0.8);
    push_row_thermal(tbl_v, 3333,2010, 60, 0.8);
}

void init_char_table()
{
    tbl_c = ThermalTable{};
    push_row_thermal(tbl_c, 256,  1000, 60, 0.9);
    push_row_thermal(tbl_c, 298,  1000, 60, 0.9);
    push_row_thermal(tbl_c, 444, 1000, 60, 0.9);
    push_row_thermal(tbl_c, 556, 1000, 60, 0.9);
    push_row_thermal(tbl_c, 644, 1000, 60, 0.9);
    push_row_thermal(tbl_c, 833, 1000, 60, 0.9);
    push_row_thermal(tbl_c, 1111,1000, 60, 0.9);
    push_row_thermal(tbl_c, 1389,1000, 60, 0.9);
    push_row_thermal(tbl_c, 1667,1000, 60, 0.9);
    push_row_thermal(tbl_c, 1944,1000, 60, 0.9);
    push_row_thermal(tbl_c, 2222,1000, 60, 0.9);
    push_row_thermal(tbl_c, 2778,1000, 60, 0.9);
    push_row_thermal(tbl_c, 3333,1000, 60, 0.9);
}
...
int main()
{
    ...
    const int    N_nodes  = 20000;
    const double L_domain = 80;
    ...
    const double T_reac[3] = {100,100,100};

    vector<double> E_over_R ={8.556E+03,2.044E+04,0.0   };
    
    vector<double> rho_v_comp = {1000,1000,1000};  // virgin yoğunluklar (her iki bileşen için aynı alındı)
    vector<double> rho_c_comp = {1000,1000,1000};
    // -------------------------------------------------------------------------
    ...
    for (int n = 1; n <= nstep; n++)
    {
        ...
        for (int it = 0; it < max_iter; it++)
        {
            ...
            a_T[0] = 1.0; d_T[0] = 2300.0;  // Twall_new;
            ...
            for (int i = 1; i < N_nodes-1; i++)
            {
                ...
                t_atime = (1000.0 * 1000.0) * t_dxi / dt;   // rho*cp*V/dt
                t_ae    = 60.0 / t_dxr;                     // k/dx_e
                t_aw    = 60.0 / t_dxl;                     // k/dx_w

                a_T[i] = t_atime + t_ae + t_aw;
                b_T[i] = t_ae;
                c_T[i] = t_aw;
                d_T[i] = t_atime * T_old[i];
            }
            ...
            // arka yuzey: adiabatik
            {
                ...
                t_atime = (1000.0 * 1000.0) * (0.5 * t_dxl) / dt;
                t_kw    = 60.0;
        ...
        if (recession_step > 0.0)
        {
            ...
            T_new[0] = 2300.0;  // T_wall_new;
            P_new[0] = P_surf;
            // k_node'u remap sonrası alpha_new ile güncelle
            for (int j = 0; j < N_nodes; j++)
            {
                rho_solid_new[j] =1000;
                alpha_eff[j] =0;
