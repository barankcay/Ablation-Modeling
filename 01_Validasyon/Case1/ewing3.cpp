...
// -----------------------------------------------------------------------------
// CMA d(rho)/dt -> alpha ODE dönüşümü için ölçekli pre-exponential:
// k = A * ((rho_v - rho_c)/rho_v)^(Psi-1) * exp(-E/RT)
// Biz burada exp kısmını zaten loop içinde hesaplıyoruz; ölçek faktörünü A'ya gömüyoruz.
// -----------------------------------------------------------------------------
    vector<double> B_eff_arr(N_comp, 0.0);

    for (int c = 0; c < N_comp; c++)
    {
        if (B_arr[c] == 0.0) { B_eff_arr[c] = 0.0; continue; }

        double rho_v = rho_v_comp[c];
        double rho_c = rho_c_comp[c];

        // fiber (C) için zaten A=0; ayrıca rho_v==rho_c ise pyrolysis yok
        if (fabs(rho_v - rho_c) < 1e-14) { B_eff_arr[c] = 0.0; continue; }

        double ratio = (rho_v - rho_c) / rho_v;   // (rho_v-rho_c)/rho_v
        B_eff_arr[c] = B_arr[c] * pow(ratio, Psi_arr[c] - 1.0);
    }
    ...
    double rho_virgin     = (1.0 - phi_v) * ( Gamma*(rho_v_comp[0] + rho_v_comp[1]) + (1.0-Gamma)*rho_v_comp[2] );
    double rho_char_total = (1.0 - phi_c) * ( Gamma*(rho_c_comp[0] + rho_c_comp[1]) + (1.0-Gamma)*rho_c_comp[2] );
    ...
    const double T_back = 1000, P_surf = 101325.0, P_back = 101325.0;
    ...
    // BASLANGIC KOSULLARI
    // =========================================================================
    vector<double>         T_old(N_nodes, 1000);
    vector<double>         T_prev(N_nodes, 1000);
    ...
    for (int n = 1; n <= nstep; n++)
    {
        ...
        for (int i = 0; i < N_nodes; i++)
            T_old[i] = 1000.0;
        if (n == 1) printf("DEBUG n=1 basliyor\n");
        // =====================================================================
        // 1. PIROLIZ
        // =====================================================================
        for (int c = 0; c < N_comp; c++)
        {
            ...
            for (int i = 0; i < N_nodes; i++)
            {
                // CMA cutoff: T <= T_reac ise reaksiyon yok
                if (T_old[i] <= T_reac[c]) {
                ...
                // güvenlik clamp
                if (pyr_anew < 0.0) pyr_anew = 0.0;
                if (pyr_anew > 1.0) pyr_anew = 1.0;
        ...
        if (n == 1) printf("DEBUG piroliz OK  aA=%f aB=%f\n", alpha_new[0][0], alpha_new[1][0]);
        // =====================================================================
        ...
        // =====================================================================
        // if (n % (int)(1.0/dt) == 0)
        //     printf("%5.0f  aeff=%.4f  aA=%.4f  aB=%.4f\n", 
        //         time, alpha_eff[0], alpha_new[0][0], alpha_new[1][0]);
